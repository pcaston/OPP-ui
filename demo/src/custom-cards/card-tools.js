...
if (!window.cardTools) {
  ...
  cardTools.checkVersion = (v) => {
    if (version < v) {
      throw new Error(
        `Old version of card-tools found. Get the latest version of card-tools.js from https://github.com/thomasloven/devcon-card-tools`
      );
    }
  };
  ...
  cardTools.deviceID = (() => {
    const ID_STORAGE_KEY = "devcon-player-device-id";
  ...
  cardTools.moreInfo = (entity) => {
    cardTools.fireEvent("opp-more-info", { entityId: entity });
  };
  ...
  cardTools.parseTemplate = (text, error) => {
    const _parse = (str) => {
      try {
        ...
        let v = cardTools.opp().states[`${parts[0]}.${parts[1]}`];
        parts.shift();
        parts.shift();
