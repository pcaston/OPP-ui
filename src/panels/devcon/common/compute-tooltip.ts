...
function computeActionTooltip(
  ...
  let tooltip =
    (isHold
      ? opp.localize("ui.panel.devcon.cards.picture-elements.hold")
      : opp.localize("ui.panel.devcon.cards.picture-elements.tap")) + " ";

  switch (config.action) {
    case "navigate":
      tooltip += `${opp.localize(
        "ui.panel.devcon.cards.picture-elements.navigate_to",
        "location",
    ...
    case "url":
      tooltip += `${opp.localize(
        "ui.panel.devcon.cards.picture-elements.url",
        "url_path",
        config.url_path
      )}`;
      break;
    case "toggle":
      tooltip += `${opp.localize(
        "ui.panel.devcon.cards.picture-elements.toggle",
        "name",
        state
      )}`;
      break;
    case "call-service":
      tooltip += `${opp.localize(
        "ui.panel.devcon.cards.picture-elements.call_service",
        "name",
        config.service
      )}`;
      break;
    case "more-info":
      tooltip += `${opp.localize(
        "ui.panel.devcon.cards.picture-elements.more_info",
        "name",
        state
      )}`;
      break;
