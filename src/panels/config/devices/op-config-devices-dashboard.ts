...
import { DeviceRowData } from "./op-devices-data-table";
import {
...
export class OpConfigDeviceDashboard extends LitElement {
            ...
            name: {
              ...
              template: (name, device: DataTableRowData) => {
                const battery = device.battery_entity
                  ? this.opp.states[device.battery_entity]
                  : undefined;
  ...
  private _batteryEntity(
    ...
    const batteryEntity = (deviceEntityLookup[deviceId] || []).find(
      (entity) =>
        this.opp.states[entity.entity_id] &&
        this.opp.states[entity.entity_id].attributes.device_class === "battery"
    );
...
declare global {
  interface HTMLElementTagNameMap {
    "op-config-devices-dashboard": OpConfigDeviceDashboard;
  }
}
