...
class OpFormCustomize extends LocalizeMixin(PolymerElement) {
  static get template() {
    return html`
  ...
  _computeSingleAttribute(key, value, secondary) {
    const config = oppAttributeUtil.LOGIC_STATE_ATTRIBUTES[key] || {
      type: oppAttributeUtil.UNKNOWN_TYPE,
    };
  ...
  getNewAttributesOptions(
    ...
    const knownKeys = Object.keys(oppAttributeUtil.LOGIC_STATE_ATTRIBUTES)
      .filter((key) => {
        const conf = oppAttributeUtil.LOGIC_STATE_ATTRIBUTES[key];
        return (
          conf &&
          (!conf.domains ||
            !this.entity ||
  ...
  selectedNewAttributeObserver(selected) {
    ...
    if (selected === this.newAttributesOptions.length - 1) {
      ...
      const attr = this._initOpenObject("", "", false /* secondary */, {
        type: oppAttributeUtil.ADD_TYPE,
      });
