...
import { UnsubscribeFunc, OppEntities } from "../../../websocket/lib";
import {
  css,
  CSSResult,
  customElement,
...
import { OpTabsSubpageDataTable } from "../../../layouts/opp-tabs-subpage-data-table";
...
export class OpConfigEntities extends SubscribeMixin(LitElement) {
  ...
  @query("opp-tabs-subpage-data-table")
  private _dataTable!: OpTabsSubpageDataTable;
  ...
  private _columns = memoize(
    (narrow, _language): DataTableColumnContainer => {
      ...
      if (narrow) {
        columns.name.template = (name, entity: any) => {
          return html`
            ${name}<br />
            ${entity.entity_id} |
            ${this.opp.localize(`component.${entity.platform}.config.title`) ||
              entity.platform}
          `;
        };
      ...
      columns.platform = {
        ...
        filterable: true,
        template: (platform) =>
          this.opp.localize(`component.${platform}.config.title`) || platform,
      };
      columns.status = statusColumn;
      ...
      for (const entry of entities.concat(stateEntities)) {
        ...
        result.push({
          ...
          name:
            ...
            this.opp.localize("state.default.unavailable"),
          unavailable,
          status: unavailable
            ? this.opp.localize(
                "ui.panel.config.entities.picker.status.unavailable"
            ...
            : this.opp.localize("ui.panel.config.entities.picker.status.ok"),
        });
      }
    ...
    showConfirmationDialog(this, {
      ...
      confirm: () => {
        this._selectedEntities.forEach((entity) =>
          updateEntityRegistryEntry(this.opp, entity, {
            disabled_by: null,
          })
        );
    ...
    showConfirmationDialog(this, {
      ...
      confirm: () => {
        removeableEntities.forEach((entity) =>
          removeEntityRegistryEntry(this.opp, entity)
        );
        this._clearSelection();
      },
    });
