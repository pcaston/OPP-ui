...
export class CastManager {
  ...
  public get castConnectedToOurOpp(): boolean {
    return (
      this.status !== undefined &&
      this.auth !== undefined &&
      this.status.connected &&
      (this.status.oppUrl === this.auth.data.oppUrl ||
        (CAST_DEV && this.status.oppUrl === CAST_DEV_OPP_URL))
    );
  }
