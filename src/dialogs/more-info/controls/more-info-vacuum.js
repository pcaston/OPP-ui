...
class MoreInfoVacuum extends PolymerElement {
    ...
    return html`
      ...
      <div hidden$="[[!supportsFanSpeed(stateObj)]]">
        <div class="horizontal justified layout">
          ...
          >
            <span
              ><iron-icon icon="opp:fan"></iron-icon>
  ...
  fanSpeedChanged(ev) {
    ...
    this.opp.callService("vacuum", "set_fan_speed", {
  ...
  onStop() {
    this.opp.callService("vacuum", "stop", {
      entity_id: this.stateObj.entity_id,
    });
  }

  onPlayPause() {
    this.opp.callService("vacuum", "start_pause", {
      entity_id: this.stateObj.entity_id,
    });
  }

  onPause() {
    this.opp.callService("vacuum", "pause", {
      entity_id: this.stateObj.entity_id,
    });
  }

  onStart() {
    this.opp.callService("vacuum", "start", {
      entity_id: this.stateObj.entity_id,
    });
  }

  onLocate() {
    this.opp.callService("vacuum", "locate", {
      entity_id: this.stateObj.entity_id,
    });
  }

  onCleanSpot() {
    this.opp.callService("vacuum", "clean_spot", {
      entity_id: this.stateObj.entity_id,
    });
  }

  onReturnHome() {
    this.opp.callService("vacuum", "return_to_base", {
      entity_id: this.stateObj.entity_id,
    });
  }
}
