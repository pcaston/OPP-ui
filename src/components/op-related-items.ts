...
export class OpRelatedItems extends SubscribeMixin(LitElement) {
        ...
        ? html`
            ...
            <ul>
              ${this._related.entity.map((entityId) => {
                const entity: OppEntity | undefined = this.opp.states[entityId];
                if (!entity) {
                  return;
                }
                return html`
        ...
        ? html`
            ...
            <ul>
              ${this._related.scene.map((sceneId) => {
                const scene: SceneEntity | undefined = this.opp.states[sceneId];
                if (!scene) {
                  return;
                }
                return html`
        ...
        ? html`
            ...
            <ul>
              ${this._related.script.map((scriptId) => {
                const script: OppEntity | undefined = this.opp.states[scriptId];
                if (!script) {
                  return;
                }
                return html`
...
declare global {
  interface HTMLElementTagNameMap {
    "op-related-items": OpRelatedItems;
  }
}
