 ...
 */
class OpPushNotificationsToggle extends EventsMixin(PolymerElement) {
    static get template() {
        return html `
      <paper-toggle-button
    ...
    async subscribePushNotifications() {
        ...
        try {
            ...
            try {
                applicationServerKey = await getAppKey(this.opp);
            }
            catch (ex) {
            ...
            await this.opp.callApi("POST", "notify.html5", {
                subscription: sub,
                browser: browserName,
                name,
            });
            ...
            console.error(err);
            this.fire("opp-notification", { message });
            this.pushChecked = false;
        }
    }
    async unsubscribePushNotifications() {
        ...
        try {
            ...
            if (!sub)
                return;
            await this.opp.callApi("DELETE", "notify.html5", { subscription: sub });
            await sub.unsubscribe();
        }
        catch (err) {
...
customElements.define("op-push-notifications-toggle", OpPushNotificationsToggle);
//# sourceMappingURL=op-push-notifications-toggle.js.map