...
import "../cards/op-card-chooser";
import "./op-demo-badge";
...
class OpCards extends PolymerElement {
  static get template() {
    return html`
          ...
          <template is="dom-repeat" items="[[cards.columns]]" as="column">
            <div class="column flex-1">
              <template is="dom-repeat" items="[[column]]" as="card">
                <op-card-chooser card-data="[[card]]"></op-card-chooser>
              </template>
            </div>
          </template>
        </div>
...
customElements.define("op-cards", OpCards);
