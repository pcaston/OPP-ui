...
const translateErr = (err) =>
  err === ERR_CANNOT_CONNECT
    ? "Unable to connect"
    : err === ERR_OPP_HOST_REQUIRED
    ? "Please enter a Open Peer Power URL."
    : err === ERR_INVALID_HTTPS_TO_HTTP
    ? html`
        Cannot connect to Open Peer Power instances over "http://".
        ${seeFAQ("https")}
      `
    : `Unknown error (${err}).`;
...
const INTRO = html`
  <p>
    Open Peer Power Cast allows you to cast your Open Peer Power installation to
...
export class HcConnect extends LitElement {
      ...
      return html`
        <hc-layout>
          <div class="card-content">
            Unable to connect to ${tokens!.oppUrl}.
          </div>
      ...
      return html`
        <hc-layout>
          <div class="card-content">
            ${INTRO}
            <p>
              To get started, enter your Open Peer Power URL and click
              authorize. If you want a preview instead, click the show demo
              button.
            </p>
            <p>
              <paper-input
                label="Open Peer Power URL"
  ...
  private async _handleConnect() {
    ...
    if (value === "") {
      this.error = "Please enter a Open Peer Power URL.";
      return;
