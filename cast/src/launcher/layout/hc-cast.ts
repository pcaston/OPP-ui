...
import { Connection, Auth } from "open-peer-power-js-websocket";
...
import "./hc-layout";
import { generateDefaultViewConfig } from "../../../../src/panels/devcon/common/generate-devcon-config";
...
class HcCast extends LitElement {
  ...
  @property() private devconConfig?: DevconConfig | null;

  protected render(): TemplateResult {
    if (this.devconConfig === undefined) {
      return html`
        ...
        ${error
          ...
          ? html`
              <p class="center-item">
                <mwc-button raised @click=${this._handleLaunch}>
                  <iron-icon icon="opp:cast"></iron-icon>
                  Start Casting
                </mwc-button>
              </p>
            `
          : html`
              ...
              <paper-listbox
                ...
                .selected=${this.castManager.status.devconPath || ""}
              >
                ${(this.devconConfig
                  ? this.devconConfig.views
          ...
          ${this.castManager.status
            ? html`
                <mwc-button @click=${this._handleLaunch}>
                  <iron-icon icon="opp:cast-connected"></iron-icon>
                  Manage
                </mwc-button>
              `
            : ""}
  ...
  protected firstUpdated(changedProps) {
    ...
    const llColl = getDevconCollection(this.connection);
    // We first do a single refresh because we need to check if there is LL
    // configuration.
    llColl.refresh().then(
      () => {
        llColl.subscribe((config) => {
          this.devconConfig = config;
        });
      },
      async () => {
        this.devconConfig = null;
      }
    );
  ...
  private async _handlePickView(ev: Event) {
    ...
    castSendShowDevconView(this.castManager, path);
  }
