...
const createAppConfig = ({ isProdBuild, latestBuild, isStatsBuild }) => {
  const config = createWebpackConfig({
    entry: {
      ...
      "opp-icons": "./src/entrypoints/opp-icons.ts",
    },
  ...
  if (latestBuild) {
    ...
    config.plugins.push(
      new WorkboxPlugin.InjectManifest({
        swSrc: "./src/entrypoints/service-worker-opp.js",
...
const createOppioConfig = ({ isProdBuild, latestBuild }) => {
  if (latestBuild) {
    throw new Error("Opp.io does not support latest build!");
  }
  const config = createWebpackConfig({
    entry: {
      entrypoint: path.resolve(paths.oppio_dir, "src/entrypoint.ts"),
    },
    outputRoot: "",
    isProdBuild,
    latestBuild,
  ...
  config.output.path = paths.oppio_root;
  config.output.publicPath = paths.oppio_publicPath;
