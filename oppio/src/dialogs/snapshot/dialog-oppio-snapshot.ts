...
class OppioSnapshotDialog extends LitElement {
  ...
  @property() private snapshot?: OppioSnapshotDetail;
  @property() private _folders!: FolderItem[];
  @property() private _addons!: AddonItem[];
  @property() private _dialogParams?: OppioSnapshotDialogParams;
  @property() private _snapshotPassword!: string;
  @property() private _restoreOpp: boolean | null | undefined = true;
  @query("#dialog") private _dialog!: PaperDialogElement;

  public async showDialog(params: OppioSnapshotDialogParams) {
    this.snapshot = await fetchOppioSnapshotInfo(this.opp, params.slug);
        ...
        <ul class="buttons">
          <li>
            <mwc-button @click=${this._downloadClicked}>
              <iron-icon icon="oppio:download" class="icon"></iron-icon>
              Download Snapshot
            </mwc-button>
          </li>
          <li>
            <mwc-button @click=${this._partialRestoreClicked}>
              <iron-icon icon="oppio:history" class="icon"> </iron-icon>
              Restore Selected
            </mwc-button>
          </li>
          ${this.snapshot.type === "full"
          ...
          <li>
            <mwc-button @click=${this._deleteClicked}>
              <iron-icon icon="oppio:delete" class="icon warning"> </iron-icon>
  ...
  private _partialRestoreClicked() {
    ...
    const data: {
      openpeerpower: boolean | null | undefined;
      addons: any;
      folders: any;
      password?: string;
    } = {
    ...
    this.opp
      .callApi(
        ...
        `oppio/snapshots/${this.snapshot!.slug}/restore/partial`,
        data
      )
      .then(
        () => {
  ...
  private _fullRestoreClicked() {
    ...
    this.opp
      .callApi(
        "POST",
        `oppio/snapshots/${this.snapshot!.slug}/restore/full`,
        data
      )
      .then(
        () => {
  ...
  private _deleteClicked() {
    ...
    this.opp
      ...
      .callApi("POST", `oppio/snapshots/${this.snapshot!.slug}/remove`)
      .then(
        () => {
          this._dialog.close();
  ...
  private async _downloadClicked() {
    ...
    try {
      signedPath = await getSignedPath(
        this.opp,
        `/api/oppio/snapshots/${this.snapshot!.slug}/download`
      );
    } catch (err) {
